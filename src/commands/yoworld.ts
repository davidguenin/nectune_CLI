...
export class YoWorld extends Command {
  ...
  async run() {
    ...
    if (args.edition && !isNaN(args.edition) ) {
      var nectuneData = await getData('https://www.nectune.com/lives/'+ args.edition  +'.json');
    }
    else{
      var nectuneData = await getData('https://www.nectune.com/lives.json');
    }
    ...
    if (nectuneData == null){
      ...
      console.log((customContent( "header_mention_left").toUpperCase() + chalk.italic.bold(dateToday.toUpperCase()) + customContent( "header_mention_right").toUpperCase() + chalk.bgBlack(customContent( "header_mention_commands"))) + '\n')
        ...
        async function logEvery2Seconds(i: number) {
          setTimeout(async () => {
            ...
            for (let i = 0 ; i < messages.length ; i++) {
              ...
              if (dateMessage > dateLessMinutes && messages[i].tag == "yoworld"){
                data.push({
                  left:"",
